  ...
  return <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          ...
          {canChangeRole && <DropdownMenuItem onClick={() => setRoleChangeDialogOpen(true)}>
          ...
          {user.status === 'active' && <DropdownMenuItem onClick={() => setDeactivationDialogOpen(true)}>
              <UserX className="mr-2 h-4 w-4" />
              Desativar Usuário
            </DropdownMenuItem>}
          
          {user.status !== 'active' && user.status !== 'terminated' && <DropdownMenuItem onClick={() => setUserManagementDialogOpen(true)}>
              <Power className="mr-2 h-4 w-4" />
              Ativar Usuário
            </DropdownMenuItem>}

          {user.status !== 'terminated' && <DropdownMenuItem onClick={() => setTerminationDialogOpen(true)} className="text-destructive focus:text-destructive">
      ...
      <UserManagementDialog user={user} open={userManagementDialogOpen} onOpenChange={setUserManagementDialogOpen} />

      <DeactivationDialog user={user} open={deactivationDialogOpen} onOpenChange={setDeactivationDialogOpen} />

      <TerminationDialog user={user} open={terminationDialogOpen} onOpenChange={setTerminationDialogOpen} />
