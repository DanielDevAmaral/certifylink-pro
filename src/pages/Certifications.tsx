...
export default function Certifications() {
  ...
  if (isLoading) {
    return <ErrorBoundary>
        <Layout>
          <PageHeader title="Gestão de Certificações" description="Controle de certificações profissionais e equivalências de serviços" />
  ...
  return <ErrorBoundary>
      ...
      <PageHeader title="Gestão de Certificações" description="Controle de certificações profissionais e equivalências de serviços">
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <CertificationForm certification={selectedCertification || undefined} onSuccess={handleFormSuccess} onCancel={() => setShowForm(false)} />
          </DialogContent>
        </Dialog>
      </PageHeader>
      ...
      <Collapsible open={showReports} onOpenChange={setShowReports}>
        <CollapsibleContent>
          <div className="mb-6">
            <ReportGenerator data={certifications} type="certifications" title="Certificações" userNames={userNames} />
          </div>
        </CollapsibleContent>
      </Collapsible>

      {/* Smart Filters */}
      <SmartFilterPanel filterConfigs={smartFilterConfigs} activeFilters={filters} onFiltersChange={handleFiltersChange} onClearFilters={() => handleFiltersChange({})} availableFunctions={availableFunctions} userNames={userNames} className="mb-6" />

      {/* Search and Status */}
      <Card className="card-corporate mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <AdvancedSearchBar placeholder="Buscar por nome, função ou responsável..." value={searchTerm} onChange={handleSearchChange} className="flex-1" showAdvancedButton={false} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {paginatedCertifications.map(certification => <Card key={certification.id} className={`card-corporate ${highlightedId === certification.id ? 'ring-2 ring-primary shadow-lg bg-primary/5' : ''}`}>
            <div className="flex items-start justify-between mb-4">
              ...
              {certification.equivalence_services && certification.equivalence_services.length > 0 && <div className="space-y-2">
                  <p className="text-sm font-medium text-foreground">Equivalências:</p>
                  <div className="flex flex-wrap gap-1">
                    {certification.equivalence_services.map((service, index) => <span key={index} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-accent text-accent-foreground">
                        {service}
                      </span>)}
                  </div>
      ...
      {paginatedCertifications.length === 0 && <EmptyState icon={Award} title="Nenhuma certificação encontrada" description={searchTerm || Object.keys(filters).length > 0 ? 'Ajuste os filtros ou termos de busca.' : 'Cadastre a primeira certificação para começar a gestão documental.'} actionLabel="Nova Certificação" onAction={() => setShowForm(true)} />}
