-- Phase 1: Security Corrections and Setup (without test data)
...
-- Create function to generate test data for logged in users
...
BEGIN
  ...
  (current_user_id, 'Certificação AWS', 'Cloud Computing', CURRENT_DATE + INTERVAL '180 days', 'valid');
  ...
  -- Expiring soon
  (current_user_id, 'Contrato de Prestação de Serviços', 'contract', 
   CURRENT_DATE + INTERVAL '10 days', 'expiring', '/docs/contrato-prestacao.pdf', false),
  -- Already expired
  (current_user_id, 'Licença de Software', 'license', 
   CURRENT_DATE - INTERVAL '3 days', 'expired', '/docs/licenca-software.pdf', false),
  -- Valid
  (current_user_id, 'Acordo de Confidencialidade', 'nda', 
