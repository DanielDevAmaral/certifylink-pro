...
-- Phase 2: Create comprehensive test data using existing user_id
DO $$
DECLARE
  test_user_id uuid;
BEGIN
  -- Get the first available user_id from profiles
  SELECT user_id INTO test_user_id FROM profiles LIMIT 1;
  
  -- Only create test data if we have a valid user
  IF test_user_id IS NOT NULL THEN
    ...
    (test_user_id, 'Certificação ISO 9001', 'Gestão da Qualidade', CURRENT_DATE + INTERVAL '15 days', 'expiring'),
    (test_user_id, 'Certificação PMBOK', 'Gestão de Projetos', CURRENT_DATE + INTERVAL '25 days', 'expiring'),
    -- Already expired
    (test_user_id, 'Certificação Antiga', 'Processo Desatualizado', CURRENT_DATE - INTERVAL '10 days', 'expired'),
    -- Valid for longer period
    (test_user_id, 'Certificação AWS', 'Cloud Computing', CURRENT_DATE + INTERVAL '180 days', 'valid');
    ...
    -- Expiring soon
    (test_user_id, 'Empresa ABC', 'Sistema de Gestão Financeira', 'João Silva', 
     CURRENT_DATE + INTERVAL '20 days', 'expiring', 150000.00, 
     CURRENT_DATE - INTERVAL '6 months', CURRENT_DATE - INTERVAL '3 months'),
    -- Already expired
    (test_user_id, 'Empresa XYZ', 'Portal de E-commerce', 'Maria Santos', 
    ...
    (test_user_id, 'Sistema de Notificações Ativo', 
